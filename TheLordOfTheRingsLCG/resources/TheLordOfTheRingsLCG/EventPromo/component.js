...
function createInterface(diy, editor, sheet) {
    ...
    let limit = 9; if (advancedControls) limit = 99;
    ...
    // TITLE LENGTH PANEL
    let TitleLength_panel = new TypeGrid();
    TitleLength_panel.setTitle(@LRL-Title-length);
	list = new Array('Short', 'Long');
	let TitleLength_control = new uiCycler('Title-length', list, bindings, FRONT);
    TitleLength_panel.place(TitleLength_control, 'hfill');
	Template_tab.place(TitleLength_panel, 'hfill');
...
function paintFront(g, diy, sheet) {
    ...
    if ($Template == 'Neutral'){
		sheet.paintImage(g, 'Neutral-'+$Title-length, 'Template');
	}else{
		sheet.paintImage(g, 'CustomSphere-'+$Title-length, 'Template');
	}
    ...
    region = diy.settings.getRegion('Title-'+$Title-length);
