...
It will also load the custom fonts and several lists (Set and 
...
*/
	...
	*/
	...
	for( let index in Eons.namedObjects.LRL.TagList	){ registerContextBarButton(Eons.namedObjects.LRL.TagList[index]) ; }//a\u00f1adir registro de error

/* Preferences dialog */
	importClass(ca.cgjennings.apps.arkham.dialog.prefs.FillInPreferenceCategory) ;
	importClass(javax.swing.JTextField );
	
	var pc = new FillInPreferenceCategory(@LRL-TheLordOfTheRingsLCG-short,PathUi+'TheLordOfTheRingsLCG-big.png') ;
	...
	pc.join() ; 
	pc.addTip(@LRL-AdvancedControls-tip-pref) ;
	...
	pc.join() ; 
	pc.addTip(@LRL-Collection-tip-pref) ;
	...
	pc.join() ; 
	pc.addTip(@LRL-PathToIcon-tip-pref) ;
//	pc.join() ; 
//	pc.addButton(
...
//	);	
	...
	if($LRL-Set==null) $LRL-Set = 'KeepValue' ; 
	pc.label(@LRL-Set) ;
	pc.join() ; 
	values = new Array('KeepValue').concat(Eons.namedObjects.LRL.DefaultIconList) ;
	values = values.concat(Eons.namedObjects.LRL.SetList) ;
	labels = new Array() ;
	for( index in values ){ labels[index] = @('LRL-'+values[index]) ; }
	pc.addDropDown('LRL-Set',labels,values) ;
	pc.join() ; 
	pc.addTip(@LRL-Set-tip-pref) ;
	
	if($LRL-Set-portrait-template==null) $LRL-Set-portrait-template = '' ; 
	pc.indent() ; 
	pc.addField('LRL-Set-portrait-template',@LRL-PathToIcon,LongText) ;
	pc.join() ; 
	pc.addTip(@LRL-PathToIcon-tip-pref) ;
//	pc.join() ; 
//	pc.addButton(
...
//			var filename = ResourceKit.showImageFileDialog( null) ;
//			if (filename!=null){ $LRL-SetUser = filename }
//		} 
//	);	
	pc.unindent() ; 
...
function gameObject(){// GAME ){
	...
	this.SetCombo = new Array() ;
	for( index in this.CollectionList ){
		let item = this.CollectionList[index] ;
		this.SetList = this.SetList.concat( 
			String(Game.get('LRL').masterSettings.get(item+'-Set-list')).split(',') 
		) ;
		list = this.DefaultIconList.concat(this.SetList) ;
		for( index in list ){
			let item = list[index] ;
			this.SetCombo[index] = ListItem( item,@('LRL-'+item),uiIcon(item) ) ;
		}
	}
	debug(3,'SetList: '+this.SetList) ;
	...
	for( index in this.CollectionList ){
		...
		encounterSetListForCurrentCollection = String(Game.get('LRL').masterSettings.get(collection+'-Set-list')).split(',') ;
...
//	this.UnicodeFont = registerFont('Sun-ExtA.ttf');
	...
	this.StyleList = new Array('LRLfont','LRLsymbols','LRLtitle','Trait','Section') ;
	...
	this.TagList = new Array(
		...
		, 'Sailing','HeadingOnCourse','HeadingOffCourse','HeadingBad','HeadingWorst'
