...
function debug(level, text) {
    if (Number($LRL-Debug) >= level) println(text);
}
...
function uiEncountersetList(bindings, sides) {
    ...
    let control = new comboBox(GO.EncountersetCombo, null);
...
function createTextBox(key, diy, sheet) {
    ...
    */
    ...
    switch (String($(key + '-textFitting'))) {
        case 'none':
            box.setTextFitting(FIT_NONE);
            break; //don't fit text
        case 'spacing':
            box.setTextFitting(FIT_TIGHTEN_LINE_SPACING);
            break;
        case 'scaling':
            box.setTextFitting(FIT_SCALE_TEXT);
            break;
        case 'both':
            box.setTextFitting(FIT_BOTH);
    ...
    for (let index in GO.IconTagList) {
        let item = GO.IconTagList[index];
        debug(5, '\nEffect tag: '+item); 
        let tag = diy.settings.get(item + '-tag');
        debug(5, 'Tag: '+tag); 
        let replacement = diy.settings.get(item + '-tag-replacement', '');
        debug(5, 'Replacement: '+replacement); 
        box.setReplacementForTag(tag, replacement);
    }

//    for (let index in GO.EffectTagList) {
//        let item = GO.EffectTagList[index];
//        debug(5, '\nEffect tag: '+item); 
//        let tag = diy.settings.get(item + '-tag');
//        debug(5, 'Tag: '+tag); 
//        let replacement = diy.settings.get(item + '-tag-replacement', '');
...
//    }
    ...
    for (index in GO.StyleTagList) {
        let item = GO.StyleTagList[index];
        debug(5, '\nEffect tag: '+item); 
        let tag = diy.settings.get(item + '-tag');
        debug(5, 'Tag: '+tag); 
        let style = diy.settings.getTextStyle(item + '-style', null);
        debug(5, 'Style: '+style); 
        box.setStyleForTag(tag, style);
    }
...
function uiNameUnique(diy, bindings, sides) {
    debug(2, '\n\tuiNameUnique');
    /*
...
function uiNameByEncounterset(diy, bindings, sides) {
    debug(2, '\n\tuiNameByEncounterset');
    /*
    Creates the component title/name control that can use the text of the
    selected Encounter Encounterset.
    */
    let grid = new TypeGrid();

    let ByEncounterset_control = new uiButtonText('ByEncounterset', diy, bindings, sides);
    let Name_control = uiName(diy, bindings, sides);
    grid.place(Name_control, 'hfill', ByEncounterset_control, '');

    return grid;
}

function uiNameParagraph(diy, bindings, sides) {
    debug(2, '\n\tuiNameParagraph');
    /*
    Creates the component title/name control. It's different from other text
    controls because it's linked to file name and other special features.
...
function uiCampaignPart(diy, bindings, sides) {
    ...
    */
    ...
    grid.place(text_control, 'hfill', ByCollection_control, '', '<html><b>'+@LRL-Part+':', '', part_control, '');
...
function writeEncountersetNumber(diy, g) {
    ...
    if (number > 0) {
        ...
        if (total > 0) {
            let separator = diy.settings.get('EncountersetOf', '');
            if (separator == '') separator = #LRL-EncountersetOf;
...
function writeNameRotated(diy, g) {
    debug(2, '\n\twriteNameRotated');
...
function writeTextByEncounterset(key, writer, diy, g) {
    ...
    */
    ...
    let encounterset = diy.settings.get('Encounterset', 'StrangeEons');
...
function writeNameByEncounterset(diy, g) {
    debug(2, '\n\twriteNameByEncounterset');
    /*
    Writes $name or #set in the $name-region.
    */
    writeTextByEncounterset('Name', Name_writer, diy, g);
}

function writeTextByCollection(key, writer, diy, g) {
...
function uiTint(key, bindings, sides) {
    ...
    */
    ...
    control.setPresets(
        @LRL-Neutral, $Neutral-tint, @LRL-Leadership, $Leadership-tint, @LRL-Lore, $Lore-tint, @LRL-Spirit, $Spirit-tint, @LRL-Tactics, $Tactics-tint, @LRL-Fellowship, $Fellowship-tint, @LRL-Baggins, $Baggins-tint, @LRL-Mastery, $Mastery-tint, @LRL-Red, $Red-tint, @LRL-Green, $Green-tint, @LRL-Blue, $Blue-tint, @LRL-Yellow, $Yellow-tint, @LRL-Magenta, $Magenta-tint, @LRL-Cyan, $Cyan-tint, @LRL-White, $White-tint, @LRL-Grey, $Grey-tint, @LRL-Black, $Black-tint
    );
...
function paintIcon(key, diy, g, sheet) {
    ...
    */
    let item = diy.settings.get(key, null); // get the icon name contained inside $key
    let image;
...
function paintPortraitShadow(key, tinter, diy, g, sheet) {
    ...
    case 'Black':
        ...
        break;
    case 'PortraitTint':
        if (diy.settings.getBoolean(key+'Tint',false)) {
...
function uiEncountersetNumber(bindings) {
    ...
    grid.place(
        '<html><b>' + @LRL-Number + ':', '', Number_control, '', @LRL-Of, '', Total_control, ''
    );
    return grid;
}
...
function paintTemplateBack(diy, g, sheet) {
    ...
    */
    ...
    if (diy.settings.get($TemplateBack + 'Back-template') != null) {
   		debug(5, '\tTemplateBack: ' + diy.settings.get($TemplateBack+'Back-template'));
    	sheet.paintImage(g, $TemplateBack+'Back-template', 'TemplateBack');
    } else sheet.paintTemplateImage(g);
}
...
function paintCustomSphereBody(diy, g, sheet) {
    ...
    */
    ...
    CustomSphereBody_tinter.setFactors(tint[0], tint[1], tint[2]); // mover a listener
    let image = CustomSphereBody_tinter.getTintedImage();
    sheet.paintImage(g, image, 'Template-region');
    ...
    if (CustomSphereBodyIcon_tinter) {
        ...
        CustomSphereBodyIcon_tinter.setImage(image); // put image into tinter
        CustomSphereBodyIcon_tinter.setFactors(tint[0], tint[1], tint[2]); // modify tinter colour
        image = CustomSphereBodyIcon_tinter.getTintedImage(); // get tinted image
...
function paintCustomSphereDecoration(diy, g, sheet) {
    ...
    */
    debug(5, '\tTint: ' + $SphereColour-tint);
    let tint = diy.settings.getTint('SphereColour-tint');
    CustomSphereDecoration_tinter.setFactors(tint[0], tint[1], tint[2]); // mover a listener
    let image = CustomSphereDecoration_tinter.getTintedImage();
    sheet.paintImage(g, image, 'Template-region');
}
...
function createTinter(key, diy) {
    ...
    */
    ...
    let tint = diy.settings.get(key+'-tintKey',key);
    tint = diy.settings.getTint(tint);
    tinter.setFactors(tint[0], tint[1], tint[2]);
...
function onClear(diy) {
    ...
    */
    for (let index in GO.LocalizableList) {
