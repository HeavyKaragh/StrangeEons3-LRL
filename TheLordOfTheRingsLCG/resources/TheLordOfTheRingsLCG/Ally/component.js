...
(it's a short text or a paragraph?), one for the painter (if it's text, which font
...
For example, card's "Type" text setting, ($Type), is defined in the user interface
...
function createInterface(diy,editor,sheet) {
    ...
    */
    ...
    diy.nameField and will prompt the user to ensure the .eon file name.
    ...
    */
    ...
    let ResourceCost_control = new uiStatIcon(
    	'ResourceCost',bindings,FRONT,
    	 9,['X','-']
    );
    let Willpower_control = new uiStatIcon(
    	'Willpower',bindings,FRONT,
    	9,['X','-']
    );
    let Attack_control = new uiStatIcon(
    	'Attack',bindings,FRONT,
    	9,['X','-']
    );
    let Defense_control = new uiStatIcon(
    	'Defense',bindings,FRONT,
    	9,['X','-']
    );
    let HitPoints_control = new uiStatIcon(
    	'HitPoints',bindings,FRONT,
    	19,['X','-']
    );
    /* 
    Most ui controls are similarly defined: $setting to link, bindings, and
    side of the component were it's shown, so that side will be updated when
    this control changes. Of course other control specific options may be 
    defined. In this case, uiStatIcon provides a numerical list that includes
    non-numerical options, like - and X. 9 and 19 determine the maximum value
    of the stat. Each stat is preceded by an icon for easy identification.
    */
    ...
    */
    ...
    /* 
    "br" (break) adds the Stats_panel to the next row of the user interface,
    that is, below the previous Title_panel. Note that 'br' wasn't needed
    for the stat controls.
    */
    ...
    */
    ...
    let Trait_control = new uiParagraphLabeled(
    	'Trait',bindings,FRONT,'line'
    );
    let Rules_control = new uiParagraphLabeled(
    	'Rules',bindings,FRONT,'big'
    );
    let Flavour_control = new uiParagraphLabeled(
    	'Flavour',bindings,FRONT,'medium'
    );
    /* 
    ...
    */
    ...
    let OptionLeft_control = new uiTextLabeled(
    	'OptionLeft',bindings,FRONT
    );
    let OptionRight_control = new uiTextLabeled(
    	'OptionRight',bindings,FRONT
    );
    /* 
    ...
    */
    ...
    if (advancedControls) list = list.concat(new Array(
    	'Baggins','Fellowship','Mastery','Custom'
    ));
    // "list" may be extended with unusual templates
    let Template_control = new uiListIcon(
    	'Template',list,bindings,FRONT
    );
    /* 
    ...
    */
    if (advancedControls) {
        ...
        let BodyIconTransparency_control = new uiTransparency(
        	'BodyIcon',bindings,FRONT
        );
    ...
    */
    ...
    let ShowCut_control = new uiButtonText(
    	'ShowCut',diy,bindings,FRONT
    );
    // a simple button with a text label
    Cutting_panel.place(ShowCut_control,'hfill');
    if (advancedControls) {
        let ShowBleeding_control = new uiButtonText(
        	'ShowBleeding',diy,bindings,FRONT
        );
    ...
    */
    ...
    let Artist_control = new uiTextLabeled(
    	'Artist',bindings,FRONT
    );
    let Portrait_control = new uiPortrait(
    	'Portrait',diy
    );
    let PortraitMirror_control = new uiPortraitMirror(
    	'Portrait',Portrait_control
    );
    ...
    */
    ...
    let CollectionNumber_control = new uiSpinnerLabeled(
    	'CollectionNumber',bindings,FRONT,999
    );
    // Custom control to change the collection number
    let CollectionInfo_control = new uiTextLabeled(
    	'CollectionInfo',bindings,FRONT
    );
    /* 
...
function paintFront(g,diy,sheet) {
    ...
    */
    ...
    $Trait : contains the text from the user interface
    $Trait-formatEnd : removes whatever is added in -format
