...
function createInterface(diy, editor, sheet) {
    ...
    let limit = 19; if (advancedControls) limit = 99; 
    let ThreatCost_control = new uiStatIcon('ThreatCost', bindings, FRONT, limit, ['X', '-']);
    limit = 9; if (advancedControls) limit = 99;
    ...
    limit = 19; if (advancedControls) limit = 99;
...
function paintFront(g, diy, sheet) {
    ...
    // STATS
    image = ImageUtils.get(PathNumberTintable + $ThreatCost + '.png');
 	if($Stats-decoration == 'Scroll'){
	    ThreatCost_tinter.setImage(image);
	    image = ThreatCost_tinter.getTintedImage();
    }else{
	    White_tinter.setImage(image);
	    image = White_tinter.getTintedImage();
	}
	region = 'ThreatCost-Scroll-region';
    ...
    if( $Template == 'Neutral' ){
    	...
    	region = new Region​( 
    		Number(region[0])+Number(regionModifier[0]), 
    		Number(region[1])+Number(regionModifier[1]), 
    		Number(region[2])+Number(regionModifier[2]), 
    		Number(region[3])+Number(regionModifier[3]) 
    	);
    }else{
