...
function paintCommon(layout, diy, g, sheet) {
    ...
    sheet.paintImage(g, $Template + '-' + layout, 'Template'); // Group icon decoration
    ...
    if (diy.settings.getBoolean('ShowBleeding')) {
        ...
        sheet.paintImage(g, 'Template-bleeding', 'Template');
    }
    if ($ShowCut != 'CutNo') {
        debug(4, '\tShowCut');
        if ($ShowCut == 'Cut') sheet.paintImage(g, 'Template-cut-' + layout, 'Template');
        if ($ShowCut == 'CutBig') sheet.paintImage(g, 'Template-cutBig-' + layout, 'Template');
    }
}
...
function paintBack(g, diy, sheet) {
    ...
    if (String(layout) == 'Title') {
        let groupRegion = 'Group-Title';
        let collectionRegion = 'Collection-Title';
        if (diy.settings.getBoolean('Layout-iconSwap')) {
            groupRegion = 'Collection-Title';
            collectionRegion = 'Group-Title';
        }
