...
function updateToPreferences(key, diy) {
    ...
    */
    ...
    if (diy.settings.getBoolean('LRL-UpdateToPreferences', false)) {
        ...
        if ((value != '') // used to avoid overwriting empty texts
        && (value != 'KeepValue') // used to avoid overwriting a value 
        ) {
            diy.settings.set(key, value);
            debug(4, '\tUpdated through Preferences: ' + value);
        }
    }
}
...
function uiTitleUnique(diy, bindings, sides) {
    debug(2, '\n\tuiTitleUnique');
    /*
    Creates the component title/name control and precedes it
    with Title-unique
 icon control.
    */
...
function formatPage(diy, g, sheet) {
    ...
    if (number > 0) {
        ...
        let total = diy.settings.get('PageTotal', 0) if (total > 0) {
...
function formatArtist(key, diy) {
    debug(2, '\n\tformatArtist');
    let output = '';
    let text = String(diy.settings.get(key, ''));
    debug(2, '\n\t' + key + ' : ' + text);
    let format = diy.settings.get(key + '-format', '');
    switch (text) {
    case null:
    ...
    default:
        if (diy.settings.get('Difficulty-' + $Difficulty)) {
            ...
            if (diy.settings.getTint($Difficulty + '-tint') == null) {
                debug(0, '\tError: Tint not or bad defined for: ' + $Difficulty + ': ' + diy.settings.get($Difficulty + '-tint'));
