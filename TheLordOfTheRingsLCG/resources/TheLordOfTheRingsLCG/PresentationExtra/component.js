...
// TEMPLATE TAB
	...
	TemplateLayout_control = new uiCycler( 'TemplateLayout' , list , bindings , [FRONT,BACK] ) ;
	Template_panel.place( 
		Template_control , 'hfill',
		@LRL-uiCycler-TemplateLayout , '' , TemplateLayout_control , 'hfill'
	) ;
	if( advancedControls ){
...
// COLLECTION TAB
	...
	let PageTotal_control = new uiSpinnerLabeled( 'PageTotal' , bindings , [FRONT,BACK] , 98 ) ;
	Other_panel.place( PageNumber_control , '' , PageTotal_control , '' ) ;
	PageIn_control = new uiButtonText( 'PageIn' , diy , bindings , [FRONT,BACK] ) ;
	Other_panel.place( PageIn_control , '' ) ;
	if( advancedControls ){
		let GameName_control = new uiParagraphLabeled( 'GameName' , bindings , [FRONT] , 'small' ) ;
